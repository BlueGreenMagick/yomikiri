...
pub fn parse_xml(xml_string: &str) -> Result<Vec<Entry>> {
...
pub fn parse_jmdict(parser: &mut Parser) -> Result<Vec<Entry>> {
...
pub fn parse_entry(parser: &mut Parser) -> Result<Entry> {
...
pub fn parse_form(parser: &mut Parser) -> Result<Form> {
...
pub fn parse_reading(parser: &mut Parser) -> Result<Reading> {
...
fn parse_sense(parser: &mut Parser) -> Result<Sense> {
    ...
    loop {
        match parser.next().ok_or("<sense> unclosed")?? {
            Event::ElementStart(tag) => match tag.name.as_str() {
                ...
                "xref" => {
                    sense.reference.push(parse_characters(parser, "xref")?);
                }
                "ant" => {
...
#[cfg(test)]
mod tests {
    use super::{parse_xml, unescape_entity, Entry, Form, Reading, Sense};
