...
import { buildAnkiField, type AnkiBuilderContext, type AnkiBuilderData } from "./ankiBuilder";
...
function generateAllFieldTemplateOptions(): TemplateFieldAndLabel[] {
  ...
  for (const form of ["as-is", "dict-form", "main-dict-form"] as const) {
    for (const style of ["basic", "furigana-anki", "furigana-html", "kana-only"] as const) {
      templates.push({
        name: "field",
        content: "word",
        form,
